...
class Dataset_base(Dataset):
    ...
    def sample_regular_extrap(self, images, poses, times):
        ...
        win_start = 100*random.randint(0,400/100-1)
        # win_start = 0
        ...
        return input_images, poses, times, mask, win_start/100
    ...
    def sampling(self, images, poses, times):
        
        # Sampling
        if not self.irregular and not self.opt.extrap:
            input_images, poses, times, mask = self.sample_regular_interp(images=images, poses=poses, times=times)
        elif not self.irregular and self.opt.extrap:
            input_images, poses, times, mask, win_start = self.sample_regular_extrap(images=images, poses=poses, times=times)
        elif self.irregular and not self.opt.extrap:
            input_images, poses, times, mask = self.sample_irregular_interp(images=images, poses=poses, times=times)
        else:
            input_images, poses, times, mask = self.sample_irregular_extrap(images=images, poses=poses, times=times)
        
        return input_images, poses, times, mask, win_start
...
class VideoDataset(Dataset_base):
    ...
    def __init__(self, opt, images=None, poses=None, times=None, train=True):
    ...
    def __getitem__(self, index):
        ...
        # Sampling
        input_images, poses, times, mask, win_start = self.sampling(images=images, poses=poses, times=times)

        # Transform
        #input_images = self.vtrans(input_images)  # return (b, c, h, w)

        return torch.tensor(input_images).float(), torch.tensor(poses).float(), torch.tensor(times).float(), torch.tensor(mask).float(), torch.tensor(win_start)
    
    def __len__(self):
        return len(self.images)


def parse_datasets(opt, imgs, poses, times, device):
    def video_collate_fn(batch, time_steps, opt=opt, data_type="train"):
        ...
        win_start = torch.stack([b[4] for b in batch])
        
        data_dict = {"data": images, "poses":poses, "times": times, "time_steps": time_steps, "mask": mask, "win_start":win_start}
    ...
    train_dataloader = DataLoader(VideoDataset(opt, imgs, poses, times, train=True),
                                      batch_size=opt.batch_size,
                                      shuffle=True,
                                      collate_fn=lambda batch: video_collate_fn(batch, time_steps, data_type="train"),
                                      generator=torch.Generator(device='cuda'))
    test_dataloader = DataLoader(VideoDataset(opt, imgs, poses, times, train=False),
                                     batch_size=opt.batch_size,
                                     shuffle=False,
                                     collate_fn=lambda batch: video_collate_fn(batch, time_steps, data_type="test"),
                                     generator=torch.Generator(device='cuda'))
