...
def render_path_extrapolate(args, render_poses, rnn_model, render_times, hwf, latent_store, chunk, render_kwargs, gt_imgs=None, savedir=None,
    ...
    times = torch.arange(0,300)/100
    time_obs = times[:2]
    #time_to_pred = render_times*5
    time_to_pred = times[2:][::2]
    c2w = render_poses#[-1]
    for i in range(1):
...
def train():
    ...
    if args.render_only or True:
        ...
        nerf_state_dict = torch.load('/hdd5/hiran/NeRF_ODE/DyNeRF-ODE_latent_dyn/logs_damp_once_together/nODE_sample_size_100_euler_all_pred_warmup_4motions_n_obs_50/pitcher_base/125000.tar')
        ...
        with torch.no_grad():
            ...
            testsavedir = os.path.join(basedir, expname, 'renderonly_newmotion_latent20_{}_{:06d}'.format('test' if args.render_test else 'path', start))
