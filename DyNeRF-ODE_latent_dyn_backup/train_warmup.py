...
def render_path_extrapolate(args, render_poses, rnn_model, render_times, hwf, chunk, render_kwargs, gt_imgs=None, savedir=None,
    ...
    times = times[::3]
    times_obs = times[:2]
    times_to_pred = times[1:]
    c2w = render_poses[-1]
    for i in range(1):
        #print(j,'/[13]')
        print(i)
        print(times_to_pred)
        latent_nexts, _ = rnn_model.next_latent(None, times_obs, times_to_pred)
...
def train():
    ...
    """nerf_coarse_state_dict = torch.load('../DyNeRF_latent_embedding_copy/logs_big_pendulam_single_vpt_512_coarse_n2f15_once/pitcher_base/400000.tar')["network_fine_state_dict"]
