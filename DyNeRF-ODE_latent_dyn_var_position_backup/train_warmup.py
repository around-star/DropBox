...
def render_path_extrapolate(args, render_poses, rnn_model, render_times, hwf, chunk, render_kwargs, gt_imgs=None, savedir=None,
    ...
    for i in range(1):
        ...
        print(i)
        print(times_to_pred)
        latent_nexts, _ = rnn_model.next_latent(None, times_obs, times_to_pred, loc=[-0.16509287059307098, 0.8372400403022766, 0.10000000149011612])
...
def create_nerf(args):
    ...
    latent_ode_model = create_LatentODE_model(512, z0_prior, 0.01, device, num_frames=args.num_frames)
    grad_vars = list(latent_ode_model.parameters())
    #print(grad_vars[0].shape)
    #exit()
    ...
    #optimizer_ode = torch.optim.Adamax(params=grad_vars_ode, lr=args.lrate)
...
def train():
    ...
    """nerf_coarse_state_dict = torch.load('/hdd5/hiran/NeRF_ODE/DyNeRF-ODE_latent_dyn_var_position/logs_damp_once_together_diff_starting_points/sample_size_100_euler_all_pred_n_obs_1_7motions/pitcher_base/115000.tar')["network_fine_state_dict"]
    ...
    render_kwargs_train["network_fn"].load_state_dict(d_nerf_state_dict_coarse)
    render_kwargs_train["network_fn"].requires_grad = False
    vid_ode_model.latent_net.load_state_dict(latent_embed_state_dict)"""
    ## Comment till here

    """nerf_state_dict = torch.load('/hdd5/hiran/NeRF_ODE/DyNeRF-ODE_latent_dyn_var_position/logs_damp_once_together_diff_starting_points/sample_size_100_euler_all_pred_n_obs_1_7motions/pitcher_base/115000.tar')
    nerf_fine_state_dict = nerf_state_dict["network_fine_state_dict"]
    nerf_coarse_state_dict = nerf_state_dict["network_fn_state_dict"]
    vid_ode_model.load_state_dict(nerf_state_dict["vid_ode"])

    render_kwargs_train["network_fn"].load_state_dict(nerf_coarse_state_dict)
    render_kwargs_train["network_fine"].load_state_dict(nerf_fine_state_dict)
    start = nerf_state_dict['global_step']
    optimizer.load_state_dict(nerf_state_dict['optimizer_state_dict'])"""
    ...
    for i in trange(start, N_iters):
        ...
        #latents, latents_to_pred, seen = vid_ode_model(batch_dict)
        if i < 0:
            latents, seen = vid_ode_model(batch_dict, warmup=True)
            total_loss = 0
            optimizer.zero_grad()
            #seen = 0
            ...
            latents, seen = vid_ode_model(batch_dict)
            #rgbs = []
            total_loss = 0
            optimizer.zero_grad()
            ...
            #frame_time = all_frame_times[seen]
            #frame_time = (args.num_frames * frame_time).int().item()
        latent = latents
        #latent_to_pred = latents_to_pred
        ...
        for param_i, param_group in enumerate(optimizer.param_groups):
            """if param_i < 43:
              ...
              param_group['lr'] = new_lrate_embedding"""
            param_group['lr'] = new_lrate
        ################################
        ...
        if i%args.i_weights==0:
            ...
            #latent_path = os.path.join(basedir, expname, '{:06d}.pkl'.format(i))
            #latent_file = open(latent_path, 'wb')
            save_dict = {
        ...
        if i%args.i_video==0 and i >= 5000:
