...
def train():
    ...
    load_dict = torch.load('/hdd5/hiran/NeRF_ODE/DyNeRF-ODE_latent_dyn_var_position/logs_ball_roll_together_diff_starting_points_wo_enc/ss_100_euler_all_pred_n_obs_1_10motions_loc_nODE_linear/pitcher_base/075000.tar')#["network_fine_state_dict"]
    ...
    times = torch.arange(0,500)/100
    time_obs = times[:1]
    time_to_pred = times
    latents, _ = latent_ode_model.next_latent(None,
                                            torch.tensor(time_obs),
                                            torch.tensor(time_to_pred), loc = angles[0])
    
    all_latents = latents.cpu().detach().numpy()
    for angle in angles[1:]:
        
        latents, _ = latent_ode_model.next_latent(None,
                                            torch.tensor(time_obs),
                                            torch.tensor(time_to_pred), loc = angle)
    ...
    """def create_colormap(color, num_steps):
        colors = [(0, 0, 0, 1)]  # Start with black
    ...
    for i in range(6):
        #cmap = create_colormap(plt.cm.tab10(i), 7)
        print(i)
        cmap = create_colormap(c[i], 500)
        plt.scatter(tsne_result[i*500:(i+1)*500, 0], 
                    tsne_result[i*500:(i+1)*500, 1], 
                    label=f'Cluster {i}', color=cmap)
    ...
    plt.savefig('tsne_ball_roll_10motions_linear/nODE_extrap_500_75k_ckpt.png')
