...
class LatentODE(nn.Module):
    ...
    def get_reconstruction(self, time_steps_to_predict, truth, truth_time_steps, latent_truth_time_steps,
        ...
        if isinstance(self.encoder_z0, Encoder_z0_ODE_RNN) or \
            ...
            #latent_embeddings = self.latent_net(latent_time_input).float()
            #latent_embeddings = torch.tensor([0]*512).float()
            #latent_embeddings = torch.nn.init.normal_(latent_embeddings, mean=0.0, std = 0.01/(512**(1/2)))
            if warmup:
            ...
            #print(angle)
            angle = self.embed_angle(angle[0])  ## Confirming a positive angle by adding 1
        ...
        print(first_point_enc_aug.shape) 
        angle = angle.unsqueeze(0).unsqueeze(0)
        #angle = torch.zeros([1,1,21])
        #print(angle.shape)
        #exit()
        #first_point_enc_aug = torch.cat([first_point_enc_aug, angle], dim=-1)
        #first_point_enc_aug = torch.zeros([1,1,27])
    ...
    def next_latent(self, latent, times_obs, times_to_pred, angle=0.2, run_backwards=True, n_traj_samples=1):
        ...
        if latent==None:
            ...
            #latent = self.latent_net(torch.tensor([0])).float()
