...
class Dataset_base(Dataset):
    def __init__(self, opt, train=True):
        ...
        print(f"[Info] regularity:{regularity} / task:{task}")
    
    def sample_regular_interp(self, images, poses, times):
        ...
        if self.opt.phase == 'train':
            ...
            poses = poses[np.arange(win_start, win_start + self.sample_size, 2), ...]
            times = times[np.arange(win_start, win_start + self.sample_size, 2), ...]
            mask = torch.ones((self.sample_size // 2, 1))
        else:
            input_images = images[win_start: win_start + self.sample_size]
            poses = poses[win_start: win_start + self.sample_size]
            times = times[win_start: win_start + self.sample_size]
            mask = torch.zeros((self.sample_size, 1))
            mask[np.arange(0, self.sample_size, 2), :] = 1

        mask = mask.type(torch.FloatTensor).to(self.device)
        return input_images, poses, times, mask
    
    def sample_regular_extrap(self, images, poses, times, angle):
        ...
        #win_start = random.randint(0, seq_len - self.sample_size - 10) if self.train else 0
        win_start = 100*random.randint(0,1000/100-1)
        #win_start = 0
        #win_start = random.randint(0, 100-self.sample_size)
        #win_start = 10*random.randint(0, 9)
        #self.sample_size = 100 - win_start

        input_images = images[win_start: win_start + self.sample_size]
        poses = poses[win_start: win_start + self.sample_size]
        times = times[win_start: win_start + self.sample_size]
        angle = angle[win_start: win_start + self.sample_size]
        mask = torch.ones((self.sample_size, 1))
        mask = mask.type(torch.FloatTensor).to(self.device)
        
        return input_images, poses, times, angle, mask, win_start/100
    
    def sample_irregular_interp(self, images, poses, times):
    ...
    def sample_irregular_extrap(self, images, poses, times):
    ...
    def sampling(self, images, poses, times, angle):
        
        # Sampling
        if not self.irregular and not self.opt.extrap:
            input_images, poses, times, mask = self.sample_regular_interp(images=images, poses=poses, times=times, angle=angle)
        elif not self.irregular and self.opt.extrap:
            input_images, poses, times, angle, mask, win_start = self.sample_regular_extrap(images=images, poses=poses, times=times, angle=angle)
        elif self.irregular and not self.opt.extrap:
            input_images, poses, times, mask = self.sample_irregular_interp(images=images, poses=poses, times=times, angle=angle)
        else:
            input_images, poses, times, mask = self.sample_irregular_extrap(images=images, poses=poses, times=times, angle=angle)
        
        return input_images, poses, times, angle, mask, win_start
...
class VideoDataset(Dataset_base):
    ...
    def __init__(self, opt, images=None, poses=None, times=None, angle=None, train=True):
    ...
    def __getitem__(self, index):
        ...
        # Sampling
        input_images, poses, times, angle, mask, win_start = self.sampling(images=images, poses=poses, times=times, angle=angle)

        # Transform
        #input_images = self.vtrans(input_images)  # return (b, c, h, w)

        return torch.tensor(input_images).float(), torch.tensor(poses).float(), torch.tensor(times).float(), torch.tensor(angle), torch.tensor(mask).float(), torch.tensor(win_start)
    
    def __len__(self):
        return len(self.images)


def parse_datasets(opt, imgs, poses, times, angle, device):
    def video_collate_fn(batch, time_steps, opt=opt, data_type="train"):
        
        images = torch.stack([b[0] for b in batch])
        poses = torch.stack([b[1] for b in batch])
        times = torch.stack([b[2] for b in batch])
        angle = torch.stack([b[3] for b in batch])
        mask = torch.stack([b[4] for b in batch])
        win_start = torch.stack([b[5] for b in batch])
        
        data_dict = {"data": images, "poses":poses, "times": times, "time_steps": time_steps, "angle": angle, "mask": mask, "win_start":win_start}
    ...
    if opt.irregular:
        ...
        if opt.extrap:
            ...
            time_steps = np.arange(0, opt.sample_size) / (opt.sample_size-1)
        else:
    ...
    train_dataloader = DataLoader(VideoDataset(opt, imgs, poses, times, angle, train=True),
                                      batch_size=opt.batch_size,
                                      shuffle=True,
                                      collate_fn=lambda batch: video_collate_fn(batch, time_steps, data_type="train"),
                                      generator=torch.Generator(device='cuda'))
    test_dataloader = DataLoader(VideoDataset(opt, imgs, poses, times, angle, train=False),
                                     batch_size=opt.batch_size,
                                     shuffle=False,
                                     collate_fn=lambda batch: video_collate_fn(batch, time_steps, data_type="test"),
                                     generator=torch.Generator(device='cuda'))
