...
def create_LatentODE_model(input_dim, z0_prior, obsrv_std, device, 
    ...
    embed_angle, angle_dim = get_embedder(10, 1, 0)
    #latent_embedder_out_dim = 512 + angle_dim
    ...
    if poisson:
        ...
        dim = latents  # OG code
        #latents_new = latents + input_dim  ## Added
        #dim = latents_new   ### Added
    ...
    # decoder_output_dim = gen_data_dim
    decoder_output_dim = 512
    decoder = Decoder(latents, decoder_output_dim).to(device) ## OG Code
    #decoder = Decoder(latents_new, decoder_output_dim).to(device)
